...
(function() {
  ...
  fmConnector.getColumnHeaders = function() {
  	var conf  = JSON.parse(tableau.connectionData);
    var fieldNames = conf.fieldNames;
    var fieldTypes = conf.fieldTypes;
  ...
  fmConnector.getTableData = function(lastRecordToken) {
    ...
    if(lastRecordToken.length == 0){
      ...
      var lastRecordId = parseInt(lastRecordToken);
      ...
      var lastRecordId = parseInt(lastRecordTokenObj.lastRecordId);
    }
  ...
  fmConnector.storeMetaData = function(metaData){
    ...
    metaData.forEach(function(meta){
      if(connectionConf.fieldNames.indexOf(meta.name) == -1){ //skip duplicated field if it is already included
        connectionConf.fieldNames.push(meta.name);
    ...
    if(connectionConf.incremental && connectionConf.fieldNames.length>0){
      ...
      connectionConf.fieldNames.push('-recordId');
      connectionConf.fieldTypes.push('int');
      //Use -recordId as default incrementalExtractColumn if incremental fetch is selected.
      tableau.incrementalExtractColumn = "-recordId";
    }
